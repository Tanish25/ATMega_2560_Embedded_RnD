...
441,450,458,467,476,485,494,503};
	...
	while(1)
	{
		for(int i=0;i<360;i++)
		{
			OCR1AL= testArray[i]/2;
			OCR1AH=(testArray[i]/2)>>8;
			if((i+120)>359)
			{
			OCR1CL =  testArray[i+120-360]/2;
     		OCR1CH =  (testArray[i+120-360]/2)>>8;
			}
			else
			{
			OCR1CL =  testArray[i+120]/2;
     		OCR1CH =  (testArray[i+120]/2)>>8;
			}
			if((i+240)>359)
			{
			OCR1BL =  testArray[i+240-360]/2;
     		OCR1BH =  (testArray[i+240-360]/2)>>8;
			}
			else
			{
			OCR1BL =  testArray[i+240]/2;
     		OCR1BH =  (testArray[i+240]/2)>>8;
			}


			
			
			OCR3AL= (testArray[i]/2-4);
			OCR3AH=(testArray[i]/2-4)>>8;
		
			if((i+120)>359)
			{
				
				OCR3CL =  testArray[i+120-360]/2-4;
	     		OCR3CH =  ((testArray[i+120-360]/2)-4)>>8;
	     	
			}
			else
			{
				OCR3CL =  testArray[i+120]/2-4;
	     		OCR3CH =  (testArray[i+120]/2-4)>>8;
	     		
			}
			if((i+240)>359)
			{
				OCR3BL =  testArray[i+240-360]/2-4;
	     		OCR3BH =  (testArray[i+240-360]/2-4)>>8;
	     	}	
			else
			{
				OCR3BL =  testArray[i+240]/2-4;
	     		OCR3BH =  (testArray[i+240]/2-4)>>8;
			}
			//_delay_ms(10);
			_delay_us(105);//changing delay to twice to reduce frequency by half
